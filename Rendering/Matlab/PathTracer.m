...
renderedImage = zeros(h,w,3);
camera = [50 52 295.6 ; normalizeVector([0 -0.042612 -1])];
camX = [w.*0.5135./h 0 0];
camY = normalizeVector(cross(camX, camera(2,:))).*fov;
    ...
    for x = 1:w
        for sy = 1:2
            for sx = 1:2
                ...
                line(x, 1) = line(x, 1) + 0.25*clamp(r(1));
                line(x, 2) = line(x, 2) + 0.25*clamp(r(2));
                line(x, 3) = line(x, 3) + 0.25*clamp(r(3));                
            end
        end
    end
    disp(y);
    renderedImage(y,:, :) = line;
end
renderedImage = imrotate(renderedImage, 180);
image(renderedImage);

function normalizedVector = normalizeVector(v)
    normalizedVector = v./norm(v); 
end

function intersection = intersectSphere(sphere, ray)
    op = sphere(2:4) - ray(1,:);
    eps = 1e-4;
    b = dot(op, ray(2,:));
...
function [out, t, id] = intersectRay(ray, sphereList)
    n = size(sphereList, 1);
    inf = 1e20;
    t = 1e20;
    id = 0;
...
function out = computeRadiance(sphereList, ray, depth, Xi)
    ...
    elseif(depth > 10)
        ...
        elseif(obj(11) == 1)
            ...
            out = obj(5:7)+f.*(computeRadiance(sphereList, [x;d], depth, Xi));
            %return

        % MIRROR
        elseif(obj(11) == 2)
            out = obj(5:7) + f.*(computeRadiance(sphereList, [x;ray(2,:)-n.*2.*dot(n,ray(2,:))], depth, Xi));
            %return
        % GLASS
        elseif(obj(11) == 3)
            ...
            if(cos2t < 0)
                out = obj(5:7) + f.*(computeRadiance(sphereList, reflRay, depth, Xi));
                %return
            else        
                ...
                if(depth > 2)
                    if(rand < P)
                        out = obj(5:7) + f.*(computeRadiance(sphereList, reflRay, depth, Xi).*RP);
                        %return
                    else
                        out = obj(5:7) + f.*(computeRadiance(sphereList, [x;tdir], depth, Xi).*TP);
                        %return
                    end
                else
                    out = obj(5:7) + f.*(computeRadiance(sphereList, reflRay, depth, Xi).*Re + computeRadiance(sphereList, [x;tdir], depth, Xi).*Tr);
                end
            end
        end
    end        
