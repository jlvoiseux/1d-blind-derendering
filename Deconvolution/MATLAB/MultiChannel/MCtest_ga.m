...
[h_est, x_est] = fbd(g, round(num_lin/obs(5)), co);
...
function [x_axis, brdf_map, l_out, closest_obs] = rendering(obs, source, brdf, num_lin, num_ang, param)
    ...
    current_x_right = current_x_left + (wall_points(2, 1) - wall_points(1, 1))/obs(5);
...
function [s_est, g_est] = fbd(d, tau, closest_obs)
    ...
    % FIBD
    [s_interf_est, g_interf_est] = fibd(d_interf, tau, 1e-1, n, T);
    g_est =  fpr(g_interf_est, tau, n, closest_obs); 
    s_est = s_interf_est;
end
...
function [s_interf_est, g_interf_est] = fibd(d_interf, tau, tol, n, T)
    ...
    for i = 1:n
        for j = 1:n
            ...
            if i == j
                g_interf_est(2:end, index) = 0;
            end
        end
    end
    alpha = [1 0]; 
    g_interf_est = g_interf_est(:);
    ...
    for i = 1:length(alpha)
        ...
        while deltaW > tol
            ...
            W0.sa = s_interf_est;            
            [Wsol,Wfval,~,~] = solve(Wprob,W0,'Options', optimoptions(@fmincon,'Display','iter', 'MaxFunctionEvaluations', 1e4, 'UseParallel',true));
            % 1.2 Assignements
            ...
            [Wsol,Wfval] = ga(Wfun, (2*tau-1)*n*n);
            Wsol = reshape(Wsol, [2*tau-1, n*n]);
            % 2.2 Assignements
            g_interf_est = Wsol;
            W2p = W2;
            W2 = Wfval;
...
function g_est = fpr(g_interf_est, tau, n, f)
    ...
    for i = 1:length(beta)       
        ...
        Y0.gi = g_est;            
        [Ysol,Yfval,~,~] = solve(Yprob,Y0,'Options', optimoptions(@fminunc,'Display','iter', 'MaxFunctionEvaluations', 1e6, 'UseParallel',true));
        g_est = Ysol;
    end
    gi = optimvar('gi', tau, n);
    ...
    X0.gi = g_est;            
    [Xsol,Xfval,~,~] = solve(Xprob,X0,'Options', optimoptions(@fminunc,'Display','iter', 'MaxFunctionEvaluations', 1e6, 'UseParallel',true));
    g_est = Xsol;
end
...
function out = computeW(s_interf_est, g_interf_est, d_interf, n, tau, alpha)
    g_interf_est = reshape(g_interf_est, [2*tau-1, n*n]);
    W = 0;
    % 1.1.1 Compute V
    V = 0;
    for k=1:n
        temp_diff = d_interf(:, get_col_num(k, k:n, n)) - conv2(s_interf_est, 1, g_interf_est(:, get_col_num(k, k:n, n)), 'same');
        temp_diff = temp_diff .* temp_diff;
        V = V + sum(sum(temp_diff));
    end
...
function out = computeX(n, g_interf_est, g_est)
    ...
    for i=1:n
        for k=1:n
            for l=i:n
               temp_diff = g_interf_est(:, get_col_num(k, k, n)) - xcorr(g_est(:,k), g_est(:,l));
               temp_diff = temp_diff .* temp_diff;
               X = X + sum(temp_diff);
            end
        end
