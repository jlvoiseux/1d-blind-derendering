...
[h_est, x_est, interf_brdf, interf_fbd, interf_g] = fbd(g, source_support_size, co, indices_xcorr, indices, interf_test); 
figure;
subplot(1, 2, 1);
stem(interf_brdf);
subplot(1, 2, 2);
...
function [s_est, g_est, s_interf_est, g_interf_est, d_interf] = fbd(d, tau, closest_obs, indices_xcorr, indices, interf_test)
    ...
    % FIBD
    [s_interf_est, g_interf_est] = fibd(d_interf, tau, 1e-6, n, T, indices_xcorr, interf_test);
...
function [s_interf_est, g_interf_est] = fibd(d_interf, tau, tol, n, T, indices, interf_test)
...
%     end
    ...
    for i = 1:length(alpha)
        ...
        while deltaW > 1e5
            ...
            %[s_interf_est, g_interf_est, g_indices] = improve(s_interf_est, g_interf_est, d_interf, false);
...
function [out1, out2, g_indices] = improve(s_interf_est, g_interf_est, d_interf, init)
...
%         if init == true
%             [~, ind] = max(d_interf(:, i));
%             g_interf_est(:, i) = 0.1*g_interf_est(:, i);
%             g_interf_est(round(ind/interv), i) = 1;
%         end
    ...
    out2 = abs(g_interf_est./max(g_interf_est));
    [~, g_indices] = max(g_interf_est, [], 1);
end
