...
[x_est, h_est, x_interf_est, h_interf_est] = blind_derendering(g, source_support_size, obs, empty_source, num_ang, num_lin, mirror_BRDF, margin);

% figure;
% subplot(1, 2, 1)
% stem(xcorr(h, 'normalized'));
% subplot(1, 2, 2)
% stem(h_interf_est);
% 
% g_interf_test = zeros(2*num_lin-1, obs_size*obs_size);
% for i=1:obs_size*obs_size
%     g_interf_test(:, i) = FastRenderingInterf(obs, interf_test(:, i), xcorr(h'), num_ang, empty_source);
% end
% figure;
% subplot(1, 2, 1)
% imagesc(xcorr(g'));
% subplot(1, 2, 2)
% imagesc(g_interf_test);
% [s_interf_est, g_interf_est] = DerenderingInterf(xcorr(g'), h_interf_est, x_interf_est, num_lin, obs_size, source_support_size, obs, empty_source, num_ang, 1e-3);

function [x_est, h_est, x_interf_est, h_interf_est] = blind_derendering(d, tau, obs, empty_source, num_ang, num_lin, mirror_BRDF, margin)
    n = length(d(:, 1));
    T = length(d(1, :));
    d = d';
    ...
    [s_interf_est, g_interf_est] = FIBD(d_interf, T, n, tau, 1e-3, obs, empty_source, mirror_BRDF, num_lin, margin);
    [s_interf_est, g_interf_est] = DerenderingInterf(d_interf, s_interf_est, g_interf_est, T, n, tau, obs, empty_source, num_ang, 1e-3);
    g_est = PhaseRetrieval(g_interf_est, tau, n);
    s_est = PhaseRetrievalAuto(s_interf_est, T);
    %[s_est, g_est] = DerenderingPhaseRetrieval(d_interf, s_est, g_est, T, n, tau, obs, empty_source, num_ang, 1e-3);
    [s_est, g_est] = DerenderingStandard(d, s_est, g_est, T, n, tau, obs, empty_source, num_ang, 1e-3);
    for i=1:n
